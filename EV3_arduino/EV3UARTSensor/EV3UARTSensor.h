...
class EV3UARTMode {
	public:
		...
		byte data_type;                   // The data type 0= 8bits, 1=16 bits, 2=32 bits, 5=float
...
class EV3UARTSensor {
	public:
		...
		EV3UARTMode* get_mode(int mode);               // Get the EV3UARTMode object for a specific sensor
		void reset();
	...
	private:
		...
		String get_data_type(int val);                 // Helper method to get the data type as a strinf
		...
		unsigned long speed;                           // The required bit rate of thesensor
		...
		EV3UARTMode* modeArray[MAX_MODES] ;            // An array of EV£UARTMode objects
